...
public: // factory
  ...
  // Constructor args mustn't contain parent ptr, because the method passes ptr
  // to the current container as a first argument of the constructor. Example:
  // Creation of a class A with a constructor A(Container* parent, int a, int b);
...
template <typename BaseType>
bool Container::remove(std::vector<std::unique_ptr<BaseType>>& collection,
                       GuiElem* el) {
