...
Layout::Layout(Container* parent, float width, float height)
    : Container(parent, width, height), m_changed(true) {}
...
void Layout::setPadding(float all) { setPadding(all, all, all, all); }
...
sf::Vector2f Layout::getEffectiveSize() const {
  return getSize() - sf::Vector2f(m_padding.left + m_padding.right,
...
void Layout::onElemRemoved(GuiElem* el) {
  m_layoutElements.erase(
      std::find(m_layoutElements.begin(), m_layoutElements.end(), el));
  m_changed = true;
