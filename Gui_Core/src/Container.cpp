...
Container::Container(Container* parent, float width, float height)
    : Container(parent) {
...
bool Container::setBackgroundImage(const std::string& filePath) {
  try {
    m_background =
        std::make_unique<Image>(this, filePath, getSize().x, getSize().y);
...
void Container::setFillColor(const sf::Color& c) { m_region.setFillColor(c); }

sf::Vector2f Container::getSize() const { return m_region.getSize(); }

sf::Vector2f Container::getPosition() const { return m_region.getPosition(); }

bool Container::removeElem(GuiElem* elem) {
  if (remove(m_elements, elem) || remove(m_controls, elem) ||
