...
int main() {
	...
	//header
	auto controlsWrapper = gui.addElem<CenterLayout>(gui.getSize().x, gui.getSize().y / 6);
	controlsWrapper->setFillColor(sf::Color(192, 192, 192, 192));
	
	auto controls = controlsWrapper->addElem<HorizontalLayout>(controlsWrapper->getSize().x / 2, controlsWrapper->getSize().y);
	...
	createButton->addListener(GuiActiveElem::PRESSED, [&mainButtons, main](const sf::Event& e) mutable {
	...
	deleteButton->addListener(GuiActiveElem::PRESSED, [&mainButtons, main](const sf::Event& e) mutable {
		if (mainButtons.size()) {
